...
def plot_times_all(df_all, level):
    ...
    ax.set(yticks=y - width, yticklabels=df_all.index.to_list())
...
def plot_sizes_all(df_all, level):
    ...
    ax.set_ylim(-1, len(df_all.index)-1 + width*2)
...
def plot_times_split_level(df_all, graphics):
    ...
    for m in dfs:
        ...
        if not df.empty: 
            ...
            ax[ax_index].set(yticks=y - width, yticklabels=df.index.to_list())
...
def main():
    ...
    for mechanism in oqs.get_enabled_kem_mechanisms():
        with oqs.KeyEncapsulation(mechanism) as kem:
  
            level = kem.details['claimed_nist_level']
    ...
    df_times = pd.read_csv('kem_mechanism_times.csv')
    
    df_all_times = pd.DataFrame(index=oqs.get_enabled_kem_mechanisms())

    df_all_times['mean_generate_keypair'] = pd.Series(df_times.groupby('kem_mechanism').mean()['generate_keypair_times'])
    df_all_times['std_generate_keypair'] = pd.Series(df_times.groupby('kem_mechanism').std()['generate_keypair_times'])
    
    df_all_times['mean_encrypt'] = pd.Series(df_times.groupby('kem_mechanism').mean()['encrypt_times'])
    df_all_times['std_encrypt'] = pd.Series(df_times.groupby('kem_mechanism').std()['encrypt_times'])
    
    df_all_times['mean_decrypt'] = pd.Series(df_times.groupby('kem_mechanism').mean()['decrypt_times'])
    df_all_times['std_decrypt'] = pd.Series(df_times.groupby('kem_mechanism').std()['decrypt_times'])
    ...
    df_sizes = pd.read_csv('kem_mechanism_sizes.csv')

    print(df_sizes)
    
    df_all_sizes = pd.DataFrame(index=oqs.get_enabled_kem_mechanisms())

    df_all_sizes['ciphertext'] = pd.Series(df_sizes.groupby('kem_mechanism').mean()['length_ciphertext'])
    df_all_sizes['public_key'] = pd.Series(df_sizes.groupby('kem_mechanism').mean()['length_public_key'])
    df_all_sizes['secret_key'] = pd.Series(df_sizes.groupby('kem_mechanism').mean()['length_secret_key'])
    df_all_sizes['shared_secret'] = pd.Series(df_sizes.groupby('kem_mechanism').mean()['length_shared_secret'])
